...
> Design notes for **edtech-competitor-price-review** — a runnable python example that A typed Python service for tracking competitor course offers and educator review decisions.
...
- **Thin client** — a ~30-line helper that owns the base URL, the auth header, and envelope unwrapping, so call sites stay readable (e.g. `infrai.ai.rerank(...)`).
- **Feature code** — the actual task: course price review.
